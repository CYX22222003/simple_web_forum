...
export default function CreateArticle(){
    ...
    return (
        ...
        <form onSubmit={(e:any) => {
            ...
            <input className="form-control" id= "title" value={title} placeholder='Enter the title'
            onChange={(e) => {setTitle(e.target.value); setObj({"title" : title, "body" : article, email_id:email_id, tag_id: tag_id});}} /><br /><br />
            <label className="form-label"><strong>Body</strong></label>
            <textarea className="form-control" id = "article" rows={10} value={article} placeholder='Enter the body'
            onChange={e => {setArticle(e.target.value); setObj({"title" : title, "body" : article, email_id:email_id, tag_id: tag_id});}}/><br />
            <button className='btn bg-danger align-item-center' type='submit' disabled={article == "" || title == ""}>
                Create New
            </button> 
            <br />
        </form>
